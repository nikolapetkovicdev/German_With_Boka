...
export default async function AdminPage({params}: {params: Promise<{locale: string}>}) {
  ...
  const payments = await prisma.payment.findMany({
    where: {status: {in: [PaymentStatus.UNDER_REVIEW, PaymentStatus.PROOF_SUBMITTED]}},
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-6">
      ...
      <AdminPanel payments={payments.map((payment) => ({id: payment.id, student: `${payment.student.firstName} ${payment.student.lastName}`, amount: payment.amount.toString(), currency: payment.currency, status: payment.status}))} bank={bank.map((item) => ({id: item.id, currency: item.currency, recipientName: item.recipientName, singleLessonPrice: item.singleLessonPrice.toString(), packagePrice: item.packagePrice.toString()}))} />
    </main>
  );
}
